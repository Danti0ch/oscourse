  ...
  )
{
  ...
  UINT32                                Nreqmod;
  UINT32                                Nmod;
  UINT32                                MaxHorizantolResolution;
  ...
  Nreqmod = 0;
  MaxHorizantolResolution = 0;
  MaxVerticalResolution   = 0;

  for (Nmod = 0; Nmod < GraphicsOutput->Mode->MaxMode; Nmod++) {
    ...
    if (!EFI_ERROR (Status)) {
      if ((Info->HorizontalResolution > MaxHorizantolResolution) &&
          (Info->VerticalResolution >= MaxVerticalResolution)) {
            Nreqmod = Nmod;
            MaxHorizantolResolution = Info->HorizontalResolution;
            MaxVerticalResolution   = Info->VerticalResolution;
      }
    }
  }

  GraphicsOutput->SetMode (GraphicsOutput, Nreqmod);
